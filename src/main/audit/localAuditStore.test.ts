...
import { LocalAuditStore } from './localAuditStore'
import { MAX_AUDIT_EVENTS as MAX_EVENTS } from './auditCore'
...
test('merge dedupes by uid and sorts newest-first', () => {
  const s = new LocalAuditStore(tmpFile())
...
test('cursors round-trip and persist across reloads', () => {
  const path = tmpFile()
  const s = new LocalAuditStore(path)
  s.setCursors({ gw2LastLogId: 99 })
  s.setCursors({ discordLastId: '123' })
  s.flush()
  expect(new LocalAuditStore(path).getCursors()).toEqual({ gw2LastLogId: 99, discordLastId: '123' })
})
